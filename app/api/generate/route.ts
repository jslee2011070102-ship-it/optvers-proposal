...
// ===== 핵심 로직: 네이버 전체 키워드 → 기회 필터링 → 쿠팡 매칭 =====

interface NaverKeywordOpportunity {
  keyword: string
...
function filterOpportunityKeywords(
  allNaverKeywords: Array<{ kw: string; vol: number }>,
  keywords: KeywordRow[]
): NaverKeywordOpportunity[] {
  const opportunities: NaverKeywordOpportunity[] = []

  for (const naverKw of allNaverKeywords) {
    const matchedKeywordRow = keywords.find(k => k.keyword === naverKw.kw)
    const productCount = matchedKeywordRow?.productCount ?? 0

    // 기회 점수 계산
    // 높은 검색량 + 낮은 상품 수 = 높은 기회점수
    const searchWeightedScore = Math.log(naverKw.vol + 1) // 검색량을 로그 스케일로
    const supplyGapScore = 100 / (productCount + 5) // 상품 적을수록 높음
    const opportunityScore = Math.round(searchWeightedScore * supplyGapScore)

    // 그레이드 판정
    let grade: 'S' | 'A' | 'B' = 'B'
    let reason = ''

    if (naverKw.vol >= 5000 && productCount <= 3) {
      grade = 'S'
      reason = `고검색(${naverKw.vol.toLocaleString()}회) + 저공급(${productCount}개) = 강한 공백`
    } else if (naverKw.vol >= 2000 && productCount <= 5) {
      grade = 'A'
      reason = `중상검색(${naverKw.vol.toLocaleString()}회) + 저공급(${productCount}개) = 기회 키워드`
    } else if (naverKw.vol >= 1000) {
      grade = 'B'
      reason = `검색 존재(${naverKw.vol.toLocaleString()}회)하나 포화도 또는 기타 요인`
    } else {
      // 검색량 낮으면 기회 대상에서 제외
      continue
    }
    ...
    opportunities.push({
      keyword: naverKw.kw,
      naverSearchVolume: naverKw.vol,
      coupangProductCount: productCount,
      opportunityScore,
      reason,
      grade,
    })
...
export async function POST(req: NextRequest) {
  try {
    ...
    // ===== 데이터 흐름 재구성: 네이버 중심 =====
    // 1. 전체 Naver 관련 키워드 수집 (stats.popularKeywords 또는 최대한 많은 키워드)
    const allNaverKeywords = stats.popularKeywords || []

    // 2. 기회 키워드 필터링
    const opportunityKeywords = filterOpportunityKeywords(allNaverKeywords, keywords)
    const opportunityS = opportunityKeywords.filter(k => k.grade === 'S')
    const opportunityA = opportunityKeywords.filter(k => k.grade === 'A')
    const topOpportunities = [...opportunityS, ...opportunityA].slice(0, 10)

    // 3. Coupang 기본 분석 데이터 (변경 없음)
    ...
    let finalVerdict = ''
    let verdictDetail = ''
    if (step1Score >= 3 && step2Pass && opportunityS.length >= 1) {
    ...
    } else if (step1Score >= 3 && (opportunityS.length >= 1 || opportunityA.length >= 1)) {
...
═══ 절대 원칙 ═══
- 데이터에 없는 수치를 창작하지 않습니다. 모든 수치에 [네이버 키워드] 또는 [쿠팡 대시보드] 출처 표기.
...
- 쿠팡 데이터: 실제 판매 제품, 가격대, 경쟁 강도, 시장 규모 (거래 실적)

네이버 기반 기회 발굴:
1. 네이버에서 높은 검색량 = 소비자 수요 존재
2. 쿠팡에 제품 부족 = 공급 공백
3. 둘의 결합 = 신제품 출시 기회
...
[네이버 인기 검색 키워드 샘플]
${allNaverKeywords.slice(0, 10).map(k => `${k.kw}: 검색 ${k.vol.toLocaleString()}회`).join(' / ')}
자동완성 패턴: ${autoKeywords.slice(0, 15).join(', ')}
...
각 슬라이드를 SLIDE_START / SLIDE_END 로 감싸세요.`
    ...
    const productGuidance = [
      ...
      ...opportunityA.slice(0, 5).map(k => `○A등급 ${k.keyword}: 네이버검색${k.naverSearchVolume.toLocaleString()}회, 쿠팡상품${k.coupangProductCount}개`),
    ].join('\n')

    const naverConsumerNeedsSummary = `
[네이버 소비자 니즈 분석]
상위 검색 키워드(전체 네이버 데이터 기반): ${allNaverKeywords.slice(0, 5).map(k => `${k.kw}(${k.vol.toLocaleString()}회)`).join(', ')}
자동완성 검색 패턴 (소비자의 실제 의도): ${autoKeywords.slice(0, 10).join(', ')}
→ 소비자들이 실제로 찾는 상품의 특징: 자동완성 키워드에서 도출

[쿠팡 공급 공백 분석]
전체 네이버 키워드 중 고검색/저공급 키워드:
${productGuidance}
→ 네이버에서는 검색되지만 쿠팡에는 부족한 제품들을 식별함
`

    const promptB = `${commonData}

[기회 키워드 상세 데이터 — 네이버 전체 + 필터링 결과]
${gapLines}

[제품 제안 근거 — 고검색/저공급 키워드 (네이버 기반)]
${productGuidance}
상위5개 평균 판매가(쿠팡): ${avgPrice.toLocaleString()}원

${naverConsumerNeedsSummary}

=== 슬라이드 3장을 작성하세요 ===

[슬라이드 5] 네이버 인기 키워드 × 쿠팡 공급 교차분석
...
- .tbl-wrap 테이블: 등급배지(.badge.green/.badge.blue) / 키워드 / 네이버검색량 / 쿠팡상품수 / 기회점수 / 선정이유
...
선택된 키워드의 "검색량은 높지만 쿠팡 제품이 3개 이하"인 상황 확인.
예: "바이브레이터" 네이버 검색 8,630회 vs 쿠팡 상품 2개만 존재 → 공급 부족 확인

STEP 3 — 네이버 니즈 + 쿠팡 공급 공백 = 제품 방향 결정
네이버 자동완성(소비자 니즈) + 쿠팡 상품 부족(기회) + 상위 제품 분석(가격대) = 제안할 제품 정의.
예: 소비자는 "방수 기능"을 원하는데, 쿠팡에는 방수 제품이 적다 → "방수 기능 추가 제품"으로 차별화
...
✕ 네이버 검색량 높지만 쿠팡 실제 거래가 전혀 없는 제품 (카테고리 오염)
...
카테고리: ${categoryName}
목표: 제안된 제품이 실제로 이 카테고리에서 팔릴 수 있는지, 네이버 수요와 쿠팡 공급 현황이 일치하는지 객관적으로 평가

평가 기준:
1. 이 제품이 ${categoryName} 카테고리에서 실제로 팔리거나 팔릴 수 있는가?
2. 네이버 소비자 수요(검색량, 자동완성)와 쿠팡 공급 부족이 일치하는가?
3. 이 카테고리 고객이 실제로 이 제품을 사겠는가? (단순 논리가 아닌 실제 구매 가능성)
4. 제품명이 논리적으로 성립하는가? (카테고리 혼용, 브랜드명 포함, 마케팅 문구 체크)
5. 서로 다른 제품 카테고리 키워드를 결합했는가? (예: 마스크 + 바이브레이터)

평가 규칙:
- 네이버 검색량 높지만 쿠팡 상품 전무/거의 없음(0~2개) → 카테고리 외부 제품 가능성 → 재검토
- 제품명에 브랜드명 포함 → 즉시 불합격
...
네이버 수요 데이터와의 일치성, 제품명 합리성, 카테고리 적합성을 중심으로 평가하세요.`
...
주의: 이전 제안이 검증 실패했습니다.
다음을 반드시 확인하세요:
- 네이버 수요 데이터(검색량, 자동완성)와 쿠팡 공급 부족이 일치해야 함
...
위 슬라이드 5, 6의 분석(네이버 수요 + 쿠팡 공급)을 바탕으로 더욱 신중하게 제품을 제안하세요.
상단: "${finalVerdict}" .callout.green 배너 + 근거 한 줄
...
SLIDE_START / SLIDE_END 로 감싸세요.`
    ...
    return NextResponse.json({
      ...
      opportunityKeywordsCount: opportunityKeywords.length,
      opportunitySCount: opportunityS.length,
      opportunityACount: opportunityA.length,
    })
  } catch (e) {
