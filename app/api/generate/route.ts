...
- 중간: .g2 또는 .g3 그리드로 2~3개 구체적 제품 제안 (.prod-card 사용)

★ 제품명 작성 규칙 (반드시 준수) ★
제품명은 키워드를 나열하는 게 아니라, 실제로 쿠팡에서 팔릴 수 있는 자연스러운 상품명이어야 합니다.

올바른 사고 순서:
  1단계 — 핵심 키워드 1개로 제품 카테고리를 파악합니다 (예: "바이브레이터" → 진동형 성인용품)
  2단계 — 보조 키워드는 그 제품의 소재/특성/사이즈를 나타내는지 판단합니다
  3단계 — 소재+카테고리+특성 순서로 자연스러운 상품명을 만듭니다

금지 사항 (이런 제품명은 절대 만들지 마세요):
  ✕ 키워드 문자열을 그대로 이어붙이기: "바이브레이터 실리콘 코멧 쉽게 사용 성인용품"
  ✕ 경쟁 브랜드명 포함: 코멧, 텐가, 안나 등 검색에 등장한 브랜드는 절대 제품명에 쓰지 마세요
  ✕ 마케팅 문구를 제품명으로: "쉽게 사용", "편리한", "안전한" 등
  ✕ 무관한 카테고리 합치기: 성인용품 + 마스크 + 일회용 세트 → 전혀 다른 제품들을 합친 것

올바른 예시:
  ✓ "실리콘 바이브레이터 10단계 진동 방수형" (소재+카테고리+특성)
  ✓ "남성용 실리콘 오나홀 소프트 타입" (타겟+소재+카테고리+타입)
  ✓ "의료용 실리콘 딜도 리얼 사이즈 M" (소재+카테고리+크기)

각 제품 .prod-rows 구성:
  - 타겟키워드: S/A등급 키워드 1개 — 검색 N회, 상품명 포함 N개만 존재 (왜 공백인지 설명)
  - 가격포지션: 상위 평균(${avgPrice.toLocaleString()}원) 대비 추천 구간
  - 소재/스펙: 구체적 소재와 핵심 스펙
  - 차별화포인트: 경쟁 제품에 없는 것 1가지만 (간결하게)
